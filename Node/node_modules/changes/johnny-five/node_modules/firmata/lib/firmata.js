...
/**
 * Handles a generic response
 * This won't be called because of the custom case for generic responses
 * @param {Board} board
 */

SYSEX_RESPONSE[GENERIC_RESPONSE] = function(board) {};
  ...
  this.sp.on("data", function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (this.currentBuffer.length === 0 && byt === 0) {
        ...
        if (this.currentBuffer[0] === START_SYSEX && SYSEX_RESPONSE[this.currentBuffer[1]] && this.currentBuffer[this.currentBuffer.length - 1] === END_SYSEX) {
          if (this.currentBuffer[1] == GENERIC_RESPONSE) {
            console.log(['sysexResponse', this.currentBuffer]);
            board.emit('sysexResponse', {code: this.currentBuffer[2], data: Board.byteArrayToLongArray(this.currentBuffer.slice(4, -1), this.currentBuffer[3]), size: this.currentBuffer[3], bufferLength: (this.currentBuffer.length - 5)});
          }
          else
...
/**
 * Call a custom function of the Arduino code
 */

Board.prototype.sysex = function(command, data, s, interval) {
  ...
  if (data != null) {
    if (s != null) {
      if (!Array.isArray(s))
        s = [s];
      for (var i = 0; i < data.length; i++)
        d = d.concat(((interval && i >= s[0] && i <= s[1]) || (!interval && s.indexOf(i) != -1)) ? Board.longToByteArray(data[i]) : data[i]);
    }
    else
      d = data; // split values greater than 255
  }
  d = d.map(function(i) { return i & 0x3ff; }); // 0x7f = 127, 0x3ff = 1023
  
  write = [START_SYSEX, command].concat(d, [END_SYSEX]);
  this.sp.write(new Buffer(write));
  console.log(['sysex', write]);
...
Board.longToByteArray = function(long, size) {
  
  if (Array.isArray(long)) {
    var array = [];
    for (var i = 0; i < long.length; i++)
      array = array.concat(Board.longToByteArray(long[i]));
    return array;
  }

  // we want to represent the input as a size-bytes array
  var byteArray = [];
...
Board.byteArrayToLong = function(byteArray) {
...
Board.byteArrayToLongArray = function(byteArray, size) {
  
  size = size || 4;
  var long = [], i = 0;

  while (i < byteArray.length) {
    long.push(Board.byteArrayToLong(byteArray.slice(i, i + size)));
    i += size;
  }
