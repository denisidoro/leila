...
var RASP = false; //OLHA AQUI DENISSSSSS
...
removeMotorLoop = setInterval(function(){
	for(var i=0; i<motors.length; i++) {
		var currentTime = Now();
		if(motors.model !== null && (currentTime - motors[i].lastContact) > 100000) {
			mout = true;
	...
	if(m.action === "init") {
		...
		port.on("data",function(d){
			...
			for(var j=0; j<packets.length; j++) {
				...
				if(p.length > 6) {
					if(pendingRegisterRead === null) {
						//Logger.log("Unexpected Packet! p: " + p.toString('hex'));
						continue;
					}
					...
					if(cs === rs) {
						...
						if(id !== pendingRegisterRead.motor.id) {
							Logger.log("Packet does not match expected sender: "+p.toString('hex') + ", (" + id + ", " + pendingRegisterRead.motor.id + ')');
							refetch();
						} else {
							//Clear The Block
							pendingRegisterRead.lastReadTime = Now();
